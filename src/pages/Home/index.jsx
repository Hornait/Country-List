...
function Home() {
  ...
  const currentCountry = (
    filtredCountries.length === 0 ? allContries : filtredCountries
  ...
  const sortContinents = (continent) => {
    const tmp = allContries.filter(
      (item) => String(item.continents) === String(continent)
    );
    setCurrentPage(1);
    setFiltredCountries(tmp);
  };
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="contries">
          <CountriesList
            contriesOnPage={currentCountry}
            Contries={allContries}
          />
        </div>
